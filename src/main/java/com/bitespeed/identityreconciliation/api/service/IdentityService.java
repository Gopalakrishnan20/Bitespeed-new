...
public class IdentityService {
    ...
    public IdentityResponse identifyContact(IdentityRequest request) {
        ...
        if(email!=null && phoneNumber!=null) {
			...
			System.out.println(foundByEmailAndPhone);
		...
		if(primaryIndicesList.size()==1 && linkedContacts.get(1).getLinkPrecedence()==Contact.LinkPrecedence.SECONDARY){
			System.out.println("already linked");
			primaryContact = linkedContacts.get(0);
			secondaryContacts = contactRepository.findByLinkedId(primaryContact.getId());
			response.setContact(primaryContact,secondaryContacts);
			return response;
		}
